...
class LinearProbingHashTable {
	...
	public void insert(String key, Book book) {
        ...
        if(books.length>0){
		for(Book it: books){
			if(it.bookNumber.equals(book.getBookNumber())){
				System.out.println("book already exizts");
				return ;
			}
		}
	}
	...
	public void printHashTable() {
		...
		for (int i = 0; i < maxSize; i++)
			if (keys[i] != null)
				System.out.println(keys[i] + " " + books[i]);
		System.out.println();
	}
}
...
public class HashTest {
	...
	public static Book getBook() throws Exception {
		...
		Book book = new Book(number, name, price);
		return book;
	}
}
